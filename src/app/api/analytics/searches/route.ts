...
/**
 * GET /api/analytics/searches?projectId=...&from=YYYY-MM-DD&to=YYYY-MM-DD
 *
 * Returns:
 * - dailyCounts: search counts per day
 * - topSearches: top search queries by frequency
...
export async function GET(request: NextRequest) {
  ...
  // Top searches — group by query string (case-insensitive)
  const topSearches = await db
    .select({
      query: sql<string>`lower(${analyticsEvents.data}->>'query')`,
      count: sql<number>`count(*)::int`,
    })
    .from(analyticsEvents)
    .where(whereClause)
    .groupBy(sql`lower(${analyticsEvents.data}->>'query')`)
  ...
  return NextResponse.json({
    dailyCounts,
    topSearches,
    totalSearches: totalRow?.count ?? 0,
  });
}
