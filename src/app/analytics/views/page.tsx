...
function PageViewsChart({ data }: { data: DailyVisitorCount[] }) {
  if (data.length === 0) {
    return (
...
function ViewsContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/analytics/views?projectId=${projectId}&from=${fromStr}&to=${toStr}`,
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-gray-500 text-xs">
          When AI agents view your docs, activity will show up here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chart */}
      <PageViewsChart data={filledCounts} />

      {/* Single table — no referrals on Views tab */}
